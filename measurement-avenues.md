...
**Status (2026-06-28): the planned two-round roadmap is complete — R1–R5 drafted (iterations 12–16); ~17 measurement branches now exist** across choice / inventory / language / calibration / informant / real-stakes / process / emotion, plus the MTMM (C1) and moral-identity (R1) integrators. Two things are simultaneously true and must not be conflated: (a) the *marginal* value of one more design doc has genuinely fallen — the binding constraint is now **build-and-validate** (a co-PI, IRB, the multi-user runtime, the κ-validation that ungates the language channel), not design; and (b) a disciplined re-scan of the literature still surfaces a small set of *genuinely distinct, validated, measurable* constructs the map does not yet cover (Round 3 below). So this is **not** exhaustion — and the honest move is neither to manufacture filler nor to falsely declare the well dry, but to continue **only** with candidates that clearly clear the bar ("a real, distinct, validated construct the instrument can actually measure"), and to stop the instant a candidate doesn't. Round 3 is curated to that bar.

---

## Round 3 — the genuinely-distinct remainder (held to a higher bar)

The two planned rounds are done; these are the constructs that survive a strict "distinct from all ~17 existing branches AND validated AND measurable here" filter. Curated, not padded — each names exactly why it is not a relabel of an existing branch, and the weakest is flagged for possible *folding-in* rather than standing alone.

- **R6. Moral conviction / metaethical objectivism** ✓ **done** — [`r6-moral-conviction.md`](r6-moral-conviction.md) (iteration 17; the **meta-stance toward one's own values** — facts-for-everyone vs. your-own; a stated objectivism probe + revealed tolerance/compromise/language signatures; R6b discriminant from R2 sacredness / R1 centrality / value-content; R6c the stated–revealed *meta-*gap; value-neutral under extra force, the branch is charged; carded). Skitka 2010 (conviction; she already enters via Hofmann 2014); Goodwin & Darley 2008 (objectivism). **The clearest gap on the map — and the design phase's last branch.**
- **R7. Moral self-regulation dynamics (licensing)** — *within a choice sequence, does a virtuous choice at t raise the odds of a lapse at t+1?* A fast **within-session sequential dependency in behavior**, categorically distinct from B4's slow value-drift over weeks; the instrument already collects the choice stream that makes it measurable. **Replication discipline is load-bearing here:** license only the meta-supported *licensing* direction (Blanken, van de Ven & Zeelenberg 2015 meta, d≈0.31, survives); treat moral *cleansing* / the "Macbeth effect" cautiously and most likely **exclude** it — Zhong & Liljenquist 2006 has documented replication failures (Earp et al. 2014), so the repo's non-replication exclusion applies. **Priority: MEDIUM** — genuine and measurable, but must be scoped to the surviving half.
- **R8. Shame- vs guilt-proneness** *(flagged for folding-in, not assumed standalone)* — the **form** of the self-conscious moral emotion (act-focused guilt → repair vs. self-focused shame → withdraw/deny/externalize; Tangney et al. 2007), as opposed to A5's **magnitude** of felt counterfactual pull/guilt. Plausibly distinct, but **overlaps A5** enough that the honest first question is discriminant: can shame-vs-guilt be separated from the A5 emotion channel at all, or is it an A5 sub-facet? **Priority: LOW** — evaluate for distinctness from A5 first; fold in if it doesn't cleanly stand apart.

Each still owes the unit / N=1 / censoring / no-composite / value-neutral disciplines and the fraud/non-replication exclusions — and R7/R8 carry an explicit extra burden (R7 the cleansing-exclusion, R8 the A5-discriminant). If a future scan yields nothing that clears this bar, *that* is the genuine stop — journal it and halt.

**Design phase closed after R6 (Dave, 2026-06-28).** R6 shipped; **R7 (licensing dynamics) and R8 (shame-vs-guilt) are deferred — not dropped** (they remain genuine, bar-clearing candidates for a later design pass). Per Dave's redirect, the loop now pivots from *designing* branches to **building and validating** what is already designed — see [`build-and-validate.md`](build-and-validate.md). The ~18 design branches (Round 1's 10 + R1–R6) are the spec; the binding constraint was always going to become implementation, and that is now the work. The design backlog (R7/R8 + anything a future scan surfaces) is paused here, ready to resume if/when the build phase wants more breadth.
