...
- **R1. Moral identity centrality** — how central *being moral* is to the self-concept (Aquino & Reed 2002 Moral Identity Scale, **already in the repo's references**). Not *what* you value but how load-bearing morality is to who-you-are — a moderator of every other branch. Reuses the inventory's identity-vs-trait split.
- **R2. Sacred / protected values** — values held as absolute, refusing tradeoffs at any price (Baron & Spranca 1997; Tetlock taboo-tradeoffs). **Re-reads the existing cost-of-virtue `never` (right-censored) responses as protected-value markers** — a "what won't you sell at any price" detector from data already collected. Near-free; the natural next build.
- **R3. Moral disengagement** — the mechanisms that switch off moral self-sanction (Bandura 1999/2002: euphemistic labeling, diffusion of responsibility, advantageous comparison, dehumanization). *How* you let yourself off the hook; overlaps the moral-language channel (A3) for detection.
- **R4. Moral attentiveness** — the chronic tendency to *perceive* the moral dimension of everyday life (Reynolds 2008, JAP). A perceptual front-end: do you even notice the ethical stakes? Measurable via what you spontaneously flag as moral (A3) + reaction to embedded moral content.
