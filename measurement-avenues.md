...
9. **B4 — value-change dynamics.** Gated on longitudinal data. ← *next*
10. **C2 — adversarial/stress conditions.** Theoretically fragile; build last, carefully.

**Checked off:** B1 → **H10** (it. 2); A1 → **H-A1** (it. 3, Phase-2); B3 → **H12** — `h12-moral-hypocrisy.md` (it. 4, MVP-1 + §16-unlock); B2 → **H11** — `h11-moral-circle-radius.md` (it. 5, MVP-1 re-analysis); A2 → **H-A2** — `h-a2-real-stakes.md` (it. 6, Phase-2 keystone); A3 → **H-A3** — `h-a3-moral-language.md` (it. 7, MVP-1 exploratory, κ-gated); C1 → **MTMM capstone** — `c1-multimethod-convergence.md` (it. 8, analysis layer, no card); A4/A5 → **process + moral-emotion** — `h-a4-a5-process-emotion.md` (it. 9, 2026-06-19, MVP-1 exploratory; A5 = a design-stage card, A4 = analysis adjunct). Locks pending Dave's review. **8 of the 10 roadmap items drafted (A4/A5 bundles two branches); only B4 (value-drift) and C2 (stress) remain — both longitudinal/fragile. The A5 card is the first to ride the automated manifest→site pipeline.**
