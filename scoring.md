...
### 19.5 R1b — meta-moderation: the gap leg + the H12 & H10 dampening legs (BUILT, directional — cohort validity checks)
...
**DEFERRED — the H11 dampening leg.** Whether internalization *also* blunts the H11 in-group/out-group parochialism gradient `β_i` (§16) is a natural R1b extension held for later, but it is **value-charged** in a way the H10/H12 legs are not: calling parochialism a "bias to dampen" would take Singer's impartialist side against H11's own explicit Singer↔Williams/MacIntyre value-neutrality (a smuggled value, CV-2), so it stays **Dave-gated** rather than auto-built. It would add two more cohort channels for a descriptive-only companion, coupling to its own §16 pipeline exactly as the H11b discriminant half does.
