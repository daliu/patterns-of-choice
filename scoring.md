...
- **N=1 interpretability.** `β_i` / `R_i` are within-person quantities on the fixed secondary axis + ordering, so they are reveal-eligible for a single user with **no cohort standardization** (contrast the cohort-only H11a/b statistics). *Shipped 2026-07-01:* this is exactly what `circleShape` computes on-device (§16.7).
...
- **No cohort statistic on-device.** H11a/b/c never leave the research analyzer; the on-device surface (§16.7) is the within-person `β_i` / `R_i` facets alone.
...
### 16.7 On-device reveal (SHIPPED 2026-07-01)

`circleShape(records)` in `poc-projection.js` computes the §16.5 N=1 reveal on-device from the runtime's already-scored circle items `{bin, score}`:

    { beta, radius, censored,                  # β_i; R_i (int bin) or null; censored true ⇔ radius null (§13.2)
      n_bins, midpoint,                        # populated-bin count; the near-bin↔axis-floor crossing threshold
      near_bin, far_bin, near_concern, far_concern, ok }

- **Exact analyzer mirror.** Same §1.5 floors as `circle_shape_by_user` (a bin enters with ≥2 items → a shape forms with ≥4 populated ordered bins; below the shape floor the JS returns `ok: false` exactly where the analyzer omits the user), same OLS slope (`olsSlope` == `_ols_slope`), same midpoint rule (near-bin concern averaged with the −1.0 axis floor), and **no declined-value guard** — the records are post-scoring, exactly as on the Python side. Locked by the `circleShape` case in `scripts/check_impl_parity.py` (fixture users — 10 finite + 2 censored radii — plus a synthetic below-floor user exercising both suppression floors).
- **Right-censoring (§13.2, load-bearing).** When concern never crosses the midpoint the radius is `null` with `censored: true` — the wide/flat impartial circle — and is **NEVER made finite**, on either side of the parity lock. `check_h11` shape-locks the analyzer companion `H11.circle_shape_reveal` with the censoring biconditional (censored ⇔ radius `None`; a finite radius is an int inside `[near_bin, far_bin]`), the ≥4-bin floor on every row, census-count coherence, and no pooled/verdict key (§13.5).
- **Value-neutrality.** Reach + steepness are described, never ranked (§16.5 — Singer's impartialism and Williams/MacIntyre's partialism are both readings); `β_i` and `R_i` stay separate facets, never a circle score.
...
**Status.** H12a + H12c + H12b (discriminant, §18.5) + the per-person `H_i` reveal quantity are BUILT (`analyze.py --hypocrisy-log` / `--h12b-log`, gated in `check_analyzer_thresholds.py` on `analysis/fixtures/sample-hypocrisy-log.json` + `sample-h12b-log.json`, Python-only so parity stays green). H12 = reliability ∧ anchor ∧ discriminant is now complete, and the **on-device `H_i` reveal is now BUILT** (§18.7, `hypocrisyAsymmetry` in `poc-projection.js`, JS↔Python parity-locked — the third shipped N=1 reveal for the H9–R6 family, after R1 §19.7 / R6 §20.7). The self–other judgment log is a **new light data-contract** (a matched pair of severity ratings per probe); real collection + its exact phrasing (avoid a leading "aren't others worse?") are runtime/design-gated and surfaced to Dave.
...
- **N=1 interpretability.** `H_i` is a within-person mean over that person's ≥ 3 matched pairs — reveal-eligible for a single user with no cohort standardization ("you judge these acts about 1 point more harshly in others than in yourself"), contrast the cohort-only H12a / H12c statistics. **Now shipped on-device** (§18.7, `hypocrisyAsymmetry` in `poc-projection.js`, parity-locked).
...
- **No cohort statistics on-device.** The on-device reveal (§18.7) is the person's own `H_i` + pair count only; H12a / H12c / H12b (reliability, the self-serving cohort anchor, the discriminant) stay analyzer-side — the device never claims a cohort direction, and never renders a per-person "holier than thou" verdict.
- **No real self–other judgment collection yet.** The reveal runs on synthetic fixtures; the probe phrasing (avoid a leading "aren't others worse?") and real collection are runtime/design-gated and surfaced to Dave (behavioral validation is Phase-2 / IRB-gated).

### 18.7 On-device `H_i` reveal (BUILT — the third shipped N=1 reveal for the H9–R6 family)

`hypocrisyAsymmetry()` / `hypocrisyPairDelta()` in `poc-projection.js` compute the §18.1 quantity on-device for a single user, mirroring `hypocrisy_asymmetry_by_user` / `_hypocrisy_pair_delta`: the **SIGNED** paired mean `H_i = mean(severity_other − severity_self)` over that person's matched pairs, `null` below the ≥ 3-pair floor (§1.5; `MIN_HYPOCRISY_PAIRS == H12_MIN_PAIRS`), a declined judgment (either side missing / non-numeric / boolean) DROPPING the pair, never imputed 0 (the §18.1 pairing lock; `typeof v === "number"` is the JS twin of the analyzer's isinstance-not-bool check), the sign preserved so harsher-on-self stays negative. The analyzer emits the companion `H12.hypocrisy_asymmetry_reveal` (per-user `{user, h, n_pairs}`, `None` below floor). `check_impl_parity.py` locks **JS == Python on every fixture participant plus a synthetic below-floor user** (signed means + pair counts + floor suppression — parity 11→12); `check_h12` asserts the reveal block's shape (both keys present, no pooled hypocrisy scalar / per-person verdict key, never scores a below-floor `H_i`, never suppresses an at/above-floor one). Value-neutral per §18.4: the reveal states magnitude and direction descriptively — harsher-on-others and harsher-on-self both described, never ranked, no "hypocrite" label (the cohort tilt is H12c, analyzer-only) — and carries the hypothetical caveat (a **stated** judgment asymmetry; no behavioral claim). Deterministic arithmetic — no bootstrap, no seed consumed.
...
**Status.** R1a (reliability for **both** facets — `R1a` internalization + `R1a_symbolization` symbolization, each gated separately, §13.5) + R1c + the two per-person facet means (`mean_internalization`, `mean_symbolization`) are BUILT (`analyze.py --identity-log`, gated in `check_analyzer_thresholds.py` on `analysis/fixtures/sample-identity-centrality-log.json`, Python-only so parity stays green). **R1b — the meta-moderation gap leg** (R1 moderating the §6 stated–revealed gap) is now BUILT too (`analyze.py --r1b-log`, gated on `sample-r1b-log.json` + `check_r1b_moderation_lock`, Python-only): R1 = **reliability (R1a) ∧ directional anchor (R1c) ∧ gap-moderation (R1b)**. The **H12 dampening leg** of R1b — internalization also dampening the §18 self–other severity asymmetry — is now BUILT too (`analyze.py --r1b-h12-log`, gated on `sample-r1b-h12-log.json` + `check_r1b_h12_dampening_lock`, Python-only); the **H10 dampening leg** — internalization also dampening the §15 observer effect (public−anonymous) — is now BUILT too (`analyze.py --r1b-h10-log`, gated on `sample-r1b-h10-log.json` + `check_r1b_h10_dampening_lock`, Python-only); and the **H11 dampening leg** — internalization also dampening the §16 parochialism gradient (concern-falls-off-with-distance steepness `p_i = −β_i`) — is now BUILT too (`analyze.py --r1b-h11-log`, gated on `sample-r1b-h11-log.json` + `check_r1b_h11_dampening_lock`, Python-only), under **Dave's 2026-07-11 value-lock** that licenses "dampening" strictly as descriptive covariance direction (β_i stays dual-read Singer↔Williams, both poles unranked). **All three R1b H10–H12 dampening legs are now complete.** The moral-identity-centrality log is a **new light data-contract** (one Likert response per item, each carrying its `facet`); real collection + exact item phrasing (the Aquino & Reed stem "It would make me feel good to be a person who has these characteristics…") are runtime/design-gated and surfaced to Dave.
...
### 19.2 R1a — facet reliability: internalization AND symbolization (BUILT)

Moral-identity centrality must be a stable trait, not occasion noise. Split each person's sessions odd/even, compute a facet mean on each half, and correlate across participants present in both halves:
...
Bootstrap: percentile method over paired `(internalization_i^{odd}, internalization_i^{even})`, pre-committed seed `20260510 + 21`. This mirrors the H10a / H11a / H12a split-half reliability construction exactly (`_odd_even_sessions` + `_pearson_r` + `_bootstrap_ci_r`).

**Both facets are reliability-tested, SEPARATELY.** The same construction runs on the **symbolization** facet as `R1a_symbolization` (`compute_r1a_symbolization_reliability`, seed `20260510 + 35`), shipped as its **own** reliability leg — the two facets' reliabilities are reported side by side and **never pooled** into one figure (§13.5), exactly as their means are (§19.1). Internalization is the private, self-definitional dimension Aquino & Reed find the more trait-stable; **symbolization** is the more situational, outward-display dimension, so clearing the same `≥ 0.40` bar is a genuine test, not a foregone conclusion. Each leg's `pre_registered_threshold_met` is **re-derived against its own CI** in `check_r1` (the gate equals the arithmetic — `met ⇔ lower CI ≥ 0.40` — so a stamped flag alone cannot satisfy it, in either direction), mirroring the `R1b_moderation` re-derivation. Together the two legs give R1 its complete two-facet reliability story.
...
### 19.5 R1b — meta-moderation: the gap leg + the H12, H10 & H11 dampening legs (BUILT, directional — cohort validity checks)
...
**BUILT — the H12 dampening leg (the first of R1b's three H10–H12 dampening legs).** Internalization should *also* dampen the **H12 self–other severity asymmetry** `H_i` (§18.1, holier-than-thou), not only the §6 over-claim gap (`analyze.py --r1b-h12-log`, gated on `analysis/fixtures/sample-r1b-h12-log.json` + `check_r1b_h12_dampening_lock`, Python-only so parity stays green):

    internalization_i = centrality_facet_by_user(identity, "internalization")   (§19.1)
    H_i               = hypocrisy_asymmetry_by_user(hypocrisy)[i]                (§18.1, SIGNED)
    r                 = corr(internalization_i, H_i)    (= the STANDARDIZED moderation slope, bivariate)
    R1b-H12 supported ⇔  UPPER 95% bootstrap CI of r  <  0   (one-sided; seed 20260510 + 36)

Supported ⇔ a more internalized moral identity predicts a **significantly** smaller (more negative) holier-than-thou asymmetry — Aquino & Reed 2002 identity→congruence applied to the Tappin & McKay 2017 / Epley & Dunning 2000 asymmetry. **Directional** (a signed-slope test, not an R²-ceiling discriminant), and the two channels are **INDEPENDENT** — internalization rides the identity log; `H_i` rides the matched-severity hypocrisy log — so there is no algebraic identity, and `check_r1b_h12_dampening_lock()` holds ONE internalization profile FIXED and flips the verdict purely through the H channel: NEGATIVE (H = −z(internalization) + noise → supported), NULL (H ⊥ internalization → not), POSITIVE (H = +z(internalization) + noise → not — the WRONG direction is one-sided rejected). It is **DISTINCT from H12b** (the discriminant regresses `H_i` on the person's OWN `[gap, cal_error]` on an R²-ceiling); here the predictor is internalization and the question is **moderation, not reducibility** — not double-counting. **COHORT-level**, never a per-person verdict: a very negative `H_i` is **harsher-on-self** (§18.4), not scored as "better," and neither internalization pole is ranked (§19.4).

**BUILT — the H10 dampening leg (the second of R1b's three H10–H12 dampening legs).** Internalization should *also* dampen the **H10 observer effect** `O_i` — the per-person public−anonymous concern shift (§15, the read the H10c anchor aggregates) — not only the §6 over-claim gap and the H12 asymmetry (`analyze.py --r1b-h10-log`, gated on `analysis/fixtures/sample-r1b-h10-log.json` + `check_r1b_h10_dampening_lock`, Python-only so parity stays green):

    internalization_i = centrality_facet_by_user(identity, "internalization")   (§19.1)
    O_i               = observer_gap_by_user(context)[i]                         (§15, SIGNED: public − anonymous)
    r                 = corr(internalization_i, O_i)    (= the STANDARDIZED moderation slope, bivariate)
    R1b-H10 supported ⇔  UPPER 95% bootstrap CI of r  <  0   (one-sided; seed 20260510 + 37)

Supported ⇔ a more internalized moral identity predicts a **significantly** smaller (more negative) observer effect — an internalized (private-regard) moral identity is by construction less contingent on being watched (Aquino & Reed 2002's internalization dimension → less observation-dependence), applied to the §15 self-presentation gap H10c already anchors as *the* observer effect. **Directional** (a signed-slope test, not an R²-ceiling discriminant), and the two channels are **INDEPENDENT** — internalization rides the identity log; `O_i` rides the context log's public/anonymous items scored through the SAME parity-locked §2 `item_score` the revealed pipeline uses (`context_item_records` → `observer_gap_by_user`) — so there is no algebraic identity, and `check_r1b_h10_dampening_lock()` holds ONE internalization profile FIXED and flips the verdict purely through the O channel: NEGATIVE (O = −z(internalization) + noise → supported), NULL (O ⊥ internalization → not), POSITIVE (O = +z(internalization) + noise → not — the WRONG direction is one-sided rejected). **COHORT-level**, never a per-person verdict: `O_i` is **dual-read** (a larger gap is norm-responsiveness OR performativity; a smaller/negative gap is authentic consistency OR context-insensitivity), so no observer pole is scored as "better," and neither internalization pole is ranked (§19.4).

**BUILT — the H11 dampening leg (the third and final of R1b's three H10–H12 dampening legs), under Dave's 2026-07-11 value-lock.** Internalization should *also* dampen the **H11 in-group/out-group parochialism gradient** — the steepness with which concern falls off with counterparty distance (§16's `β_i`, the OLS slope of circle-radius concern on distance bin) — not only the §6 over-claim gap, the H12 asymmetry, and the H10 observer effect (`analyze.py --r1b-h11-log`, gated on `analysis/fixtures/sample-r1b-h11-log.json` + `check_r1b_h11_dampening_lock`, Python-only so parity stays green):

    internalization_i = centrality_facet_by_user(identity, "internalization")   (§19.1)
    β_i               = circle_shape_by_user(circle)[i]["beta"]                 (§16, SIGNED slope of concern on bin index)
    p_i               = −β_i                     (parochialism STEEPNESS, positive = parochial, impartial ≈ 0)
    r                 = corr(internalization_i, p_i)    (= the STANDARDIZED moderation slope, bivariate)
    R1b-H11 supported ⇔  UPPER 95% bootstrap CI of r  <  0   (one-sided; seed 20260510 + 39)

The **sign convention** (§16): the distance-bin index rises with distance, so a parochial person's concern falls off with distance (`β_i < 0`) and an impartial person's is flat (`β_i ≈ 0`); scoring `p_i = −β_i` makes parochialism a positive magnitude and keeps this leg's gate **byte-identical** to the H10/H12 family (upper CI < 0). Supported ⇔ a more internalized moral identity predicts a **significantly** flatter circle of concern (Aquino & Reed 2002's internalization dimension × §16). **Directional** (a signed-slope test, not an R²-ceiling discriminant), and the two channels are **INDEPENDENT** — internalization rides the identity log; `β_i` rides the circle log's counterparty-distance items scored through the SAME §2 `circle_radius` axis the revealed pipeline uses (`circle_item_records` → `circle_shape_by_user`) — so there is no algebraic identity, and `check_r1b_h11_dampening_lock()` holds ONE internalization profile FIXED and flips the verdict purely through the β channel: NEGATIVE (p = −z(internalization) + noise → supported), NULL (p ⊥ internalization → not), POSITIVE (p = +z(internalization) + noise → not — the WRONG direction is one-sided rejected). **COHORT-level**, never a per-person verdict: `β_i` is **dual-read** under H11's own Singer↔Williams/MacIntyre value-neutrality (a steeper gradient = special obligations / loyalty & partiality (Williams) OR a failure of impartial concern (Singer); a flat gradient = impartial beneficence (Singer) OR cold detachment from near ties (Williams)), so no parochialism pole is scored as "better," and neither internalization pole is ranked (§19.4).

**On the value-lock (why this was long DEFERRED).** Framing parochialism as a "bias to dampen" was previously held out (CV-2) because it can read as taking Singer's impartialist side against H11's explicit value-neutrality. **Dave's 2026-07-11 decision** licenses the word "dampening" here **strictly as the direction of a cohort-level descriptive covariance** (internalization flattens the gradient), NOT a claim that impartiality is morally superior; the dual-read above keeps **both** poles unranked and the reveal descriptive-only, so the neutrality discipline is preserved. This mirrors the H10 leg's dual-read exactly, on a fourth independent (circle) channel.
...
- **No two-tailed / per-person moderation.** R1b's gap leg is one-sided (upper CI < 0 only — the wrong-direction POSITIVE cohort is rejected, proven by `check_r1b_moderation_lock`) and cohort-level; it is never a per-person "your identity moderates your gap" verdict, and a very negative gap is modesty, not scored better (§19.5). The H12, H10, and H11 dampening legs are now all BUILT on the same one-sided directional gate (proven by `check_r1b_h12_dampening_lock` / `check_r1b_h10_dampening_lock` / `check_r1b_h11_dampening_lock`); the H11 leg (§19.5) is descriptive-covariance-only under Dave's 2026-07-11 value-lock, with `β_i` dual-read so neither pole is ranked. All three R1b H10–H12 dampening legs are complete.
- **No cohort statistics on-device.** The N=1 facet reveal now ships in `poc-projection.js` (§19.7), but both facets' R1a reliability (`R1a` + `R1a_symbolization`), the R1c directional anchor, and R1b moderation are **cohort-level** reads that deliberately stay OFF-device — they need the validation cohort, are not per-person, and would mislead if shown as an N=1 verdict. Only the two per-person facet means cross to the device.
...
### 22.4 A4b — the discriminant half (BUILT); the on-device reveal (SHIPPED); revision capture (DEFERRED)

Built earlier: the conflict primitive + the A4a reliability gate + the synthetic reliability fixture. Built since: **H-A4b, the discriminant half**, and **the on-device `conflict(i, domain)` reveal** (SHIPPED 2026-07-10, below). Status by leg:
...
- **The on-device reveal + its parity lock — SHIPPED (2026-07-10).** A4 was **parity-gated in principle** (unlike A3, whose non-determinism put it permanently outside parity — conflict is a deterministic arithmetic transform of logged RT, so it *belongs* in parity once revealed), and the reveal now ships: `conflictReads()` in `poc-projection.js` mirrors the analyzer's `conflict_by_user_domain` under the JS↔Python parity lock (§22.6), taking the parity gate **16→17**. Legitimate as an N=1 **on-device** read because the within-person z-score and per-domain mean are **self-contained** — they never touch another person's records, so the cohort-filtered analyzer value equals the solo on-device value **exactly** (verified 1e-15). What stays cohort-level and OFF-device is A4a's per-domain **reliability** and A4b's cohort **discriminant** (both need the cohort, are not per-person, would mislead as an N=1 verdict); only `conflict(i, domain)` — the person's own relative effort — crosses to the device. This closes the on-device reveal program: A4 is the **8th** shipped N=1 reveal (after R1 §19.7 / R6 §20.7 / H12 §18.7 / H10 §15.7 / H11 §16.7 / R2 §17.7 / H9 §14.8).
...
### 22.6 The on-device conflict reveal (SHIPPED 2026-07-10)

`conflictReads()` in `poc-projection.js` computes the §22.1 quantity on-device for a single user, mirroring the analyzer's `conflict_by_user_domain`: person i's **within-person relative effort per domain** — `conflict(i, domain)` = the mean over that person's domain-d items of `rt_z(i, item)`, where `rt_z` is the z-score of `response_time_ms` **residualized on `[prompt_chars, presented_position]`** (OLS via plain-stdlib Gaussian elimination), with `was_timeout` and the **timed quick-fire** set (CV-1) excluded, then z-scored **within** the person. The port carries the residualizer faithfully: `_solveLinear` (partial-pivot Gaussian elimination, first-max tie-break to match Python's `max`, `< 1e-12` singular guard → `null`), `_olsResiduals` (normal equations XᵀXβ = Xᵀy, `null` when underdetermined / rank-deficient → the caller falls back to within-person **mean-centering**), the A4 accessors, and the `A4_MIN_ITEMS = 4` floor. The population SD (`÷ N`, not the module's sample `sd()`) and the `sd == 0 → conflict 0.0` degenerate branch match the analyzer exactly.

**Why this is legitimate as an N=1 on-device read.** The within-person z-score and the per-domain mean **never touch another person's records** — every step is scoped to person i alone. So the on-device value computed from the device's own log equals the analyzer's cohort-filtered value **exactly** (verified to 1e-15): the reveal needs no cohort standardization. This is why A4a's per-domain **reliability** and A4b's cohort **discriminant** stay OFF-device (they *are* cohort statistics, not per-person, and would mislead as an N=1 verdict) while `conflict(i, domain)` crosses to the device.

`check_impl_parity.py` locks **JS == Python on every fixture participant plus three synthetic edge users** that pin the discrete branches a float tolerance can't catch: `zz-below-floor` (3 < 4 scorable items → below `A4_MIN_ITEMS` → no cells → Python-absent ⇔ JS `ok:false`), `zz-rankdef` (constant `presented_position` → rank-deficient design → `_ols_residuals` null → mean-centering fallback with SD > 0), and `zz-flat` (constant `prompt_chars` → rank-deficient AND constant `response_time_ms` → residuals exactly 0 → `sd == 0` → `conflict 0.0`) — taking the parity gate **16→17**. Value-neutral per §22.3: the reveal states **magnitude and direction of effort descriptively** — "your *harm* choices took more deliberation than your *fairness* choices" — never a moral-framework label (slow ≠ deontological; Bago & De Neys 2019), never a virtue ranking (effortful virtue is not worse than easy virtue), never a pooled cross-person scalar (§13.5), and carries the RT-only / exploratory caveat. Deterministic arithmetic — no bootstrap, no seed consumed. This is the **8th and final** shipped N=1 reveal for the H9–R6 / A-series family, completing the on-device reveal program.
