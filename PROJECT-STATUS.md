...
**Last updated:** 2026-07-03 (five on-device reveals shipped 2026-07-01 — R1 moral-identity facets, R6 metaethical objectivism, H12 self-other severity asymmetry, H10 cross-situational consistency, H11 moral-circle shape — each locked to a Python analyzer companion; the cross-implementation parity gate grew 9 → 14 cases and the Node suite to 182 assertions. Prior pass 2026-06-09: the local-first runtime app BUILT & shipped on daliu.github.io; corpus 61 + 5 recurring-character arcs + 9 H8 pairs; H9 added as the 9th hypothesis; analyzer aligned with the on-device projection + the parity gate introduced)
...
- `check_impl_parity.py` (`make check` / `make check-parity`) — a cross-implementation parity gate asserting the JS on-device projection and the Python analyzer agree on item scoring and on every on-device reveal. Green (14 cases: 11 item-scoring incl. secondary-axis, + 5 reveal locks — centralityFacets, objectivismReads, hypocrisyAsymmetry, contextVariability, circleShape).
...
Crossed (the local-first runtime app — **BUILT & shipped** at `daliu.github.io/patterns-of-choice/app.html`): a dependency-free, no-build vanilla-JS PWA realizing the architecture's event-sourced model on-device. It has an event-sourced engine (append-only IndexedDB log, `runtime/poc-runtime.js`), a deterministic on-device projection/scorer (`runtime/poc-projection.js`), a content bundle, an offline service worker, and 182 Node assertions across 4 test files (all green). User-facing: a first-run onboarding flow, the full recurring-character arc player (the H8 mechanism made interactive), card-sort + quick-fire/narrative daily sessions, a colorful animated reveal, a shareable reveal portrait (SVG→canvas image), and a cautious type-to-confirm profile-wipe. **Everything on-device; nothing sent — this is the local-first, individual-benefit tool.** (The runtime source lives in the separate public repo, alongside the demos.)
...
- [~] Runtime app — **the local-first single-user app is BUILT & shipped** (`app.html`, on-device, 182 tests green). The *cohort-capable* pilot runtime (adds multi-user sync + scheduling on top) is still designed-only (`runtime-architecture.md`, DECISIONS §18)
...
Last comprehensive review: 2026-06-09 (local-first runtime app built & shipped; corpus → 61 + 5 recurring-character arcs + 9 H8 pairs; H9 added as the 9th hypothesis; analyzer aligned with the on-device projection + cross-implementation parity gate added). Incremental update 2026-07-03: five on-device reveals (R1/R6/H12/H10/H11) shipped 2026-07-01 with analyzer parity (gate 9 → 14, Node suite → 182); see "Since the 2026-06-09 pass" above.
