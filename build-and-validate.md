...
**What's implemented = the core validity spine (H2–H7). What's spec-only = every measurement branch the design phase added (H8–H12, the A-series, R1–R6).** That gap is this backlog.
...
1. **H9 — self-calibration (`scoring.md §14`).** Fully-specced math; N=1; reuses the gap + bootstrap + the **window-b fixtures already present** (test–retest). The cleanest first build. `e_i^p = pred − rev`, `cal_bias`, `cal_error`; axis + cost-of-virtue channels never pooled; censoring-aware; reactivity netting.
