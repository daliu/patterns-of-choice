...
## Iteration 49 — 2026-07-20 — DAVE UNBLOCKED R3 → the moral-disengagement channel is OPEN: R3a (κ-free choice-channel profile reliability) is BUILT — the FIRST new-branch build since the H8–R6 family closed, reliability-only, §1.4 honored in shape

**What reopened the loop.** Iterations 47–48 declared the buildable-without-Dave surface empty and surfaced the specific unblock decisions. Dave acted on **Iter-47 item #2** (design-lock a non-language detection contract for R3 → *"then R3a is a clean R2a-shaped build"*) and item #1's reframe (the **AI-panel-for-volume κ** policy), verbatim: *"AI panel for volume, report human numbers too for independence checks but don't let those be blockers to progress. R3, scope and build guilt axis, then ratify."* That is a direct authorization to build R3 first — it licenses the **paired disengaging-vs-owning framing** item contract Iter 47 flagged as the open design lock, which turns R3a into exactly the R2a-shaped reliability build Iter 47 predicted. This is the loop's **designed** reopen path (Dave makes a surfaced call → the leg it gates becomes buildable), and R3 is the **first NEW measurement branch** opened since the H8–R6 / A-series family completed.

**The branch: R3a — moral-disengagement choice-channel profile reliability (§23.1–§23.2).** Moral disengagement (Bandura 1999/2002) is the machinery that **deactivates moral self-sanction** so a person violates a standard **without guilt** — the negative pole of the A5 felt-guilt axis. It is read as a **profile**: *which* of the **8 mechanisms** across four loci (reconstrue the ACT — moral-justification / euphemistic-labeling / advantageous-comparison; obscure AGENCY — displacement / diffusion of responsibility; disregard the HARM — distortion-of-consequences; devalue the TARGET — dehumanization / attribution-of-blame) a person reaches for. R3a is the **κ-free** first leg: **reliability of the profile**, nothing more.

**The primitive (κ-free, the crux).** `D_i(mechanism)` = the rate at which person *i* picks the self-serving **disengaging** framing over the responsibility-**owning** one on **same-act** paired items tagged that mechanism (`framing_choice ∈ {disengage, own}`, ≥3 parsed choices per (user, mechanism) cell; an unparseable/absent choice is **dropped**, never scored as owning). `disengagement_profile_by_user_mechanism` returns `{(user, mechanism): rate}` — a **profile**, never a pooled scalar. The choice channel needs **no coder**, so — unlike R3's deferred *language* leg — it carries **no κ gate at all**; Dave's AI-panel-κ / human-κ-as-caveat policy governs that later language tier, not R3a.

**What R3a claims — and, load-bearingly, what it does NOT.** R3a certifies **RELIABILITY only, never validity.** The §1.4 central problem — *disengagement is indistinguishable in FORM from legitimate justification* (the spec's own "hardest part... most likely to mismeasure") — is honored **in the shape of the build**: R3a scores the disengaging option only because each item is a self-serving / standard-preserving reframe **by construction**, not by inferring bad faith. The full discriminant (self-serving + inconsistent + post-hoc + standard-preserving) is R3's **DEFERRED** validity leg (needs the H12 self–other tell + response latency). And **R3b** (§1.3, the load-bearing gap↔guilt decoupling, `corr(D_i, A5 felt-pull) < 0`) is **DEFERRED until A5 exists** — which is precisely Dave's **next** queued build ("scope and build guilt axis").

**What it adds (full pipeline, `analyze.py` + gate).** `compute_r3a_profile_reliability(records)` — splits each person's sessions **odd/even**, recomputes `D_i(mechanism)` in each half, correlates the two halves **across people, per mechanism** by reusing `_domain_test_retest_r` (mechanism as the key), lower 95% bootstrap CI ≥ **`R3A_RELIABILITY_FLOOR = 0.40`** (the **exploratory** bar, as A4a/R2a); `any_met` iff ≥1 mechanism clears. Wired end-to-end: `R3_MECHANISMS` (the 8), `R3A_SEED_OFFSET = 40` (consumes the seed **band +40..+47**, one per mechanism via `_domain_test_retest_r`'s `seed_offset + i`; **next free +48**), `R3_MIN_ITEMS_PER_MECHANISM = 3`, the `_r3_is_disengage` / `_r3_user` / `_r3_session` accessors, CLI `--r3-log`, the `hypotheses["R3"]` payload block (an inlined per-mechanism serializer keyed `"mechanism"`, + `n_disengagement_cells` / `n_participants`), and `render_r3_result` (value-neutral; states "RELIABILITY ≠ VALIDITY", names the mechanisms, "never a pooled amount"). Gate: **`check_r3`** (rejects any pooled `disengagement_score` / `mechanism_score` **or** valence `culpability` / `guilt_score` / `bad_faith` / `manipulativeness` key via `_scan`; re-derives each cell's `met ⇔ ci_low ≥ 0.40` against its own arithmetic) + the two-sided **`check_r3_disengagement_lock()`** — **7 assertions all green**: always-disengage `D=1.0` / always-own `0.0`; the ≥3 min-items floor drops a 2-choice cell; an unparseable framing dropped; the reliable corpus clears (`any_met` True); the **same** corpus flipped-at-retest fails (`any_met` False); the render is value-neutral (names mechanisms, RELIABILITY≠VALIDITY, DEFERRED); no-pool / no bad-faith verdict. Fixture `analysis/fixtures/sample-r3-log.json` (16 users × 6 sessions × 8 mechanisms × 4 items = **3072 records**, a stable latent per-mechanism disengaging propensity → **7 of 8** mechanisms clear, diffusion at CI-low ≈ 0.31 so the 0.40 bar is genuinely per-mechanism discriminating), generated by the scratchpad `gen_r3.py` (`Random(719)`, **NOT committed**) which **bite-tests** RELIABLE-clears **and** SCRAMBLED-fails before writing the static JSON. Lock corpus builder `Random(4041)`.

**What it honors.** **§1.4 in the SHAPE** (reliability ≠ validity; the discriminant DEFERRED, not assumed). **§13.5 no-pool** (per-(user, mechanism) cells + a per-mechanism reliability vector; never a "disengagement score" — the §4-rejected headline; the `POOLED`/valence guards hold). **Value-neutrality with EXTRA force** — a reframe is **not** automatically culpable (much of what these mechanisms name is ordinary legitimate reasoning); the render names mechanisms descriptively and **never emits a bad-faith verdict**. **N=1 interpretability** — `D_i` is reveal-eligible for a single user, but the **on-device reveal is DEFERRED** (a later increment, as R2/H9 shipped theirs after their reliability legs); this iteration is **cohort-level, Python-only → parity stays 17/17**. **Two-sided reliability** (the flipped-at-retest corpus fails). **PROPOSE-not-lock** — the DECISIONS lock is proposed as **§29** (A3 §26, A4 §27, **H8 §28**), correcting an initial §28 collision with H8's slot; scoring.md places R3 at sequential section **§23** (the design doc's notional §28 is its *scoring-section* number, already taken).

**What shipped.** `make check` **GREEN** at exit (validate 66 + all threshold locks + `R3: ✓ R3a 7/8 mechanism(s) clear, 128 cells × 16 participants` + `r3-disengagement` **7/7** + parity **17 passed, 0 failed**). Docs flipped spec-only→BUILT: **new `scoring.md §23`** (the PROPOSED R3 section — numbering note, §23.1 primitive, §23.2 the R3a gate BUILT, §23.3 the no-pool / no-bad-faith / RELIABILITY≠VALIDITY wall, §23.4 built/deferred legs, §23.5 what it does not compute); `r3-moral-disengagement.md` (build-status note + §1.2 marked ✅ BUILT); `build-and-validate.md` (new **item 11** + the "what's implemented" summary R3→built + the A3 sub-bullet refined [R3 choice channel κ-free, only its language leg κ-gated] + a new Needs-Dave R3 surface line + the §29 lock + the §23 cross-ref). Committed + pushed to `daliu/patterns-of-choice` `main`.

**Boundary after this iteration — NOT empty; Dave's queue has the next two builds.** Per Dave's explicit order — **R3 → A5 guilt axis → ratify** — the next build is **A5 (the felt-guilt / emotion axis)**, now Dave-authorized just as R3's framing contract was; it needs an **affect-probe elicitation design + a reactivity control** (probing affect changes affect) and earns **one design-stage public card** — I will scope those and build the synthetic scoring, surfacing (not guessing) the elicitation-wording + card decision. Building A5 in turn unblocks **R3b** (the gap↔guilt decoupling) and R3's A5-emotion legs. After A5: **ratify** the pending pre-registration locks into DECISIONS (R2c per-person, H8 §28 reconciliations, §26 A3 κ-gate, §27 A4 no-framework wall, and now §29 R3). R3's own remaining legs stay gated: the §1.4 discriminant (H12 + latency), the language tier (AI-panel κ), the Moore self-report, the on-device `D_i` reveal. If this cron fires again, the correct next action is **A5**, not a re-derivation of exhaustion.

---

## Iteration 48 — 2026-07-12 — DAVE UNBLOCKED R1b-H11 → the parochialism-dampening leg is BUILT (the third & final of R1b's H10–H12 dampening legs), under a descriptive-covariance value-lock

**What reopened the loop.** Iteration 47 declared the buildable-without-Dave surface empty and surfaced five unblock decisions. Dave acted on **#4** (the R1b-H11 value-neutrality call, verbatim: *"10. parochialism acceptable to frame as dampening"*). That is precisely the ONE built-family leg Iter 47 flagged as needing only a values decision, not new data — so this iteration builds it. This is the loop's **designed** reopen path (Dave makes a surfaced call → the leg it gates becomes buildable), not a re-derivation of exhausted work.

**The branch: R1b-H11 parochialism-dampening leg (§19.5).** Internalization should *also* flatten the H11 in-group/out-group parochialism gradient — the steepness with which concern falls off with counterparty distance (§16's `β_i`, the OLS slope of circle-radius concern on the distance-bin index) — just as it dampens the §6 over-claim gap (R1b gap leg), the H12 self–other asymmetry (H12 leg), and the H10 observer effect (H10 leg). With this leg, **all three R1b H10–H12 dampening legs are complete.**

**The sign-convention insight that kept the gate byte-identical.** The distance-bin index rises with distance, so a parochial person's concern *declines* with distance → `β_i < 0` (steeper = more parochial); impartial → `β_i ≈ 0`. Dampening means internalization *flattens* the circle → `β_i` rises toward 0. To reuse the H10/H12 family's exact one-sided gate (`upper 95% CI of corr < 0`), the leg scores **`p_i = −β_i`** (parochialism *steepness*, positive = parochial, impartial ≈ 0); then dampening = `corr(internalization, p_i) < 0` — the same `R1B_MODERATION_CEILING = 0.0`, same `R1B_MIN_PARTICIPANTS = 8`, its own seed `20260510 + 39`. Unlike the right-censored radius `R_i`, `β_i` is always finite, so `p_i` needs no censoring bookkeeping.

**What it adds (full pipeline, `analyze.py` + gate).** `compute_r1b_h11_dampening(circle_entries, identity_records, tag_map, dist_map)` — parses `β_i` internally via the same parity-locked §2 `circle_radius` axis the revealed pipeline uses (`circle_item_records` → `circle_shape_by_user`), regresses `p_i = −β_i` on `internalization_i` (§19.1), returns the standardized-slope `r` + bootstrap CI + `supported ⇔ (ci_high < 0)`. Wired end-to-end: `R1B_H11_SEED_OFFSET = 39`, CLI `--r1b-h11-log` (loads its own `dist_map` since `--circle-log` need not be present), the `R1.R1b_h11_dampening` payload block, the `render_r1_result` param + render/insufficient-data lines, and the `check_r1` re-derivation (`supported ⇔ ci_high < ceiling` on the shipped payload). Gate: **`check_r1b_h11_dampening_lock()`** — 7 assertions on N=40 cohorts built through the REAL centrality + circle pipelines, ONE internalization profile held FIXED, verdict flipped purely through the independent β channel: NEGATIVE (`p = −z(intern)+noise` → SUPPORTED, `r≈−0.84`), NULL (`p ⊥ intern` → not, `r≈+0.00`), POSITIVE (`p = +z(intern)+noise` → not, one-sided reject, `r≈+0.91`); exact-CI gate on all three; `<8`-participant floor → None; cohort-level value-neutrality. Fixture `analysis/fixtures/sample-r1b-h11-log.json` (12 participants across the {identity, circle} channels, SUPPORTED, `r≈−0.88`), generated by the scratchpad `gen_r1b_h11.py` (NOT committed; realizes `β_i` by quantizing a per-user linear `concern(bin)` onto achievable hospitality(+1)/boundaries(−1) item-score means). Seeds neg=1/null=48/pos=1 found by scratchpad search; +39 registered in the offset registry (next free after R2c's +38).

**What it honors.** **Value-neutrality under Dave's lock** — the word "dampening" is licensed strictly as the *direction of a cohort-level descriptive covariance* (internalization flattens the gradient), NOT a claim that impartiality is morally superior; `β_i` stays **dual-read** under H11's own Singer↔Williams/MacIntyre neutrality (a steeper gradient = special obligations / loyalty & partiality (Williams) OR a failure of impartial concern (Singer); a flat gradient = impartial beneficence (Singer) OR cold detachment from near ties (Williams)), so **neither pole is ranked** and the reveal stays descriptive-only (§19.4). **§13.5 no-pool** (internalization is its own facet mean; no composite centrality scalar; the `POOLED` guard holds). **Cohort-level, never a per-person verdict** — like the other three R1b legs, it is analyzer-side only, **no on-device reveal → Python-only, parity stays 17/17**. **Directional/one-sided** (the wrong-direction POSITIVE cohort is rejected). **PROPOSE-not-lock** — the value framing is recorded as Dave's 2026-07-11 decision in `scoring.md §19.5` / `build-and-validate.md` item 6, not auto-locked into DECISIONS.

**What shipped.** `make check` **GREEN** at exit (validate 66 + all threshold locks + the new `r1b-h11-dampening` 7/7 + parity **17 passed, 0 failed**; 194 ✓, 0 ✗). Docs flipped DEFERRED→BUILT: `scoring.md` §19.5 (new BUILT subsection with the formula, sign-convention, independence, dual-read + the value-lock rationale), the §19 Status line, §19.6; `build-and-validate.md` item 6 (header date list + BUILT callout + Deferred clause collapsed to the centrality-log-collection item only); the stale "H11 stays DEFERRED" notes in the `analyze.py` H10 docstring and the `check_r1b_h10_dampening_lock` docstring. Committed + pushed to `daliu/patterns-of-choice` `main`.

**Boundary after this iteration.** The buildable-without-Dave surface is **empty again** — H11 was the last built-family leg needing only a values call. The residual is the same κ/design/runtime/A5-gated set from Iter 47 (H8b, R6c, §21.5 A3 halves, A4 revision, R3/R4/R5 detection contracts). Dave's **second** direction — reframe the κ inter-rater validation as a **panel of flagship LLM raters** (Fable 5 max, ChatGPT 5.6 Sol Ultra, …) across prompt-perspectives instead of ~200 human gold codes — potentially ungates R3/R4/R5 language channels + R6c + the A3 halves, but building that rater harness is a substantial new design/infra piece, not a cheap-clean leg; per Dave's instruction it is **filed as a GitHub issue** (methodological caveat surfaced: inter-LLM κ shares training data, so it does not establish the same rater independence as inter-human κ). If this cron fires again before Dave acts, STOP with a pointer here — do not manufacture filler.

---

## Iteration 47 — 2026-07-11 — BUILDABLE-WITHOUT-DAVE BACKLOG EXHAUSTED (R3/R4/R5 scoped in full; each gated on a Dave design-lock, the κ≥0.70 human-rater validation, or an unbuilt upstream branch) → NO code increment; surfacing the specific unblock decisions rather than manufacturing filler

**What this iteration did.** Read the three spec-only NEW branches in full — `r3-moral-disengagement.md`, `r4-moral-attentiveness.md`, `r5-moral-typecasting.md` — to scope the "cheapest-clean of R3/R4/R5" the Iter 46 note pointed at, then rigorously re-verified the BUILT-family residual via a `scoring.md` DEFERRED sweep. **Verdict: there is no leg left that is buildable synthetically without a Dave decision first.** `make check` was confirmed green at entry (validate 66 + thresholds all ✓ + parity **17 passed, 0 failed**) and left **untouched**; HEAD stays `ae25c72` (Iter 46 R2c). Per the mandate's explicit clause — *"if you genuinely run out of buildable-without-Dave work, say so in the journal and stop — do not manufacture filler"* — this iteration ships **no code** and instead records the boundary + the decisions that reopen it.

**Why R3/R4/R5 are NOT cheap-clean-synthetic (each needs a Dave gate BEFORE any scoring can be honestly built).**
- **R3 (moral disengagement).** Its one κ-free leg is **R3a** — profile reliability of `D_i` (which of Bandura's 8 mechanisms a person reaches for), §1.2 split-window test–retest, lower 95% CI ≥ 0.40, an exact R2a-shaped build; §3 A2 confirms the *choice-derived* part is **parity-gated, not κ-gated**. BUT its detection channel — "a subset of items offering a **disengaging vs. a responsibility-owning framing of the same act**" — is a **new item contract no scenario file defines and the runtime does not emit**. Authoring it (which mechanisms are even probe-able "without inducing the very disengagement you measure," §6 open questions; §3 A1 calls it "light authoring") is an **open design decision**. Building scoring for a channel that does not exist yet is a premature design guess → **Dave's lock**. Worse, R3 = R3a ∧ **R3b**, and R3b (the load-bearing gap↔guilt decoupling) is **BLOCKED on A5** (the emotion branch — unbuilt, itself needs an affect-probe elicitation card), so even a perfect R3a would ship only a **reliability shell** of an incompletable branch. And §1.4 (justification *vs.* disengagement — the spec's own "hardest part... most likely to mismeasure") needs H12 data + κ-coded language.
- **R4 (moral attentiveness).** **R4b has the clean R2c/R6b discriminant SHAPE** (attentiveness ⊥ [§5 value-importance, R1 centrality], model R² upper CI < 0.50) — the exact pattern I built five times. But its **OUTCOME channel is the blocked one**: attentiveness is detected from the spontaneous moral-framing **rate** (A3 corpus → **κ-gated**) + **incidental salience** (a NEW paradigm §6 flags as "design and validate"). No existing behavioral primitive emits "attentiveness," so the discriminant's predictors are built but its dependent variable is not.
- **R5 (moral typecasting).** The spec's own framing: *"the most speculative branch on the map... closer to a future direction with a tentative read than a near-ready measure."* Detection is all indirect — κ-gated typecasting language + choice feature-weighting that needs **dedicated agent/patient role-assignment items the corpus lacks** — and R5a reliability is *"itself contingent on the detection carrying signal"* (§6 = a pilot question). Nothing here is cleanly synthetic.

**The BUILT-family sweep confirms the same boundary.** A `scoring.md` DEFERRED grep enumerates the entire residual of the H8–R6 family, and every item crosses a gate before it re-reads anything: **H8b** (§9.4 — needs the §9.3 attachment instrument, i.e. new NPC-cast items) · **R1b-H11** (§19.5 — *value-charged*: framing parochialism `β_i` as a "bias to dampen" smuggles Singer's impartialism against H11's own Singer↔Williams/MacIntyre neutrality, CV-2) · **R6c** (§20.5 — revealed tolerance/compromise + objectivist-language → **κ**) · **§21.5 A3 cohort-coupled halves** (**κ**) · **A4 answer-revision capture** (§22.4 — **runtime**-gated open question). None re-reads an existing parity-locked primitive without first crossing a Dave/κ/design line.

**The distinction that decides it.** Every clean leg this loop shipped (Iters ~33–46: H9b/H10b/H12b/R6b/R2c discriminants, the R1b gap/H10/H12 dampening legs, R1a/R6a reliabilities) **RE-READ a channel the instrument already emits** — CoV break-points, the §5 card-sort, centrality/objectivism Likerts, RT, severity ratings. R3/R4/R5 each require **INVENTING a new detection channel** (disengagement-framing choices / an incidental-salience paradigm / dyadic role-assignment items) or the κ-validated language coder. Synthetic fixtures with known ground truth validate the SCORING MACHINERY for a **real** primitive; they cannot conjure the primitive itself. Inventing a detection contract is precisely the class the mandate says to **"surface to Dave rather than guess,"** and to **"PROPOSE, not auto-lock."**

**Surfaced to Dave — any ONE of these reopens the loop (highest-leverage first).**
1. **κ≥0.70 human-rater validation of the A3 language coder** (~200 human codes). Single highest-leverage unlock — ungates the PRIMARY channel of R3 (disengagement language), R4 (spontaneous-framing rate), R5 (typecasting language), R6c (objectivist-language meta-gap), and the §21.5 A3 cohort halves all at once.
2. **Design-lock a NON-language detection contract** for one branch, making its choice/behavioral sliver synthetically buildable κ-free: **R3** — the paired disengaging-vs-owning framing item set + which of Bandura's 8 mechanisms are probe-able (§6) → then R3a is a clean R2a-shaped build; **R4** — the incidental moral-salience paradigm (§6) → then R4a/R4b/R4c follow; **R5** — the dyadic agent/patient role-assignment items → then R5a becomes testable if the pilot shows signal.
3. **Build A5 (emotion branch) first** — needs an affect-probe elicitation card/design — to unblock R3b (the load-bearing gap↔guilt decoupling) and the A4 emotion legs.
4. **R1b-H11 value-neutrality call** — the ONE built-family leg needing only a values decision, not new data: may parochialism (`β_i`, §16) be framed as a "bias to dampen" (takes Singer's side), or must it stay value-neutral (leg stays deferred)?
5. **Pre-registration DECISION locks awaiting arbitration** (propose, never auto-lock): R2c per-person-`|P_i|` vs. per-cell-LPM (Iter 46); the H8 §28 reconciliations; every real-collection item-phrasing lock.

**Re-examination (prompted by a follow-up "continue") — the boundary held and sharpened.** Stress-tested the most promising escape hatch: build **R3a / R4a as κ-free self-report-scale reliabilities**, exactly as R1a was built on Aquino-Reed 2002 and R6a on Goodwin-Darley 2008 (settled, cited, validated Likert scales → clean synthetic reliability builds). Re-read both specs' §1 in full. **Both explicitly REJECT the self-report route as primary:** R4 §1.5 (the reactivity problem — "attentiveness is the construct you most easily destroy by measuring... a dedicated probe cannot be the primary measure"); R3 §1.5 + §4 ("a self-report-only measure (rejected — desirability)"; the choices+language are "the harder-to-fake channel and the primary one"). So neither can fall back to a Reynolds-2008 / Moore-2012 Likert the way R1/R6 leaned on their scales. Their one κ-free channel is a **novel authored instrument** (R3's paired disengaging/owning framing items, §3 A1 "light authoring") whose design is an **open §6 question** (R3 §6 Q3 "which mechanisms are even probe-able without inducing the very disengagement you measure"; §6 Q1 "can the load-bearing §1.4 justification-vs-disengagement discriminant be detected at the item level *at all*"). Authoring it = making the design decision. **The decisive criterion, sharpened:** a reliability leg is cheap-clean-buildable iff it instantiates a **settled published instrument** (R1a/R6a) *or* re-reads an **existing runtime channel** (R2a/R2c/R6b/H\*b/R1b-legs); it is NOT buildable when it would require **authoring a novel instrument whose design the spec flags as open** (R3a/R4a) — that is Dave's lock. Both re-examined legs fall on the Dave side. Exhaustion stands.

**Operational note for the next fire — do NOT re-derive this.** The buildable-without-Dave surface is **empty as of Iteration 47**. If this cron fires again before Dave acts on an item above, the correct action is to **STOP again** with a one-line pointer to this entry, **not** to manufacture a new data contract to stay busy. New buildable work appears only after Dave makes one of the decisions above (or authors a new branch spec). This is the loop reaching its **designed boundary** (18 branch specs + their κ-free legs all implemented + gated), not a failure — the spec→implementation gap for everything buildable synthetically is now closed.

---

## Iteration 46 — 2026-07-10 — R2 · R2c SACRED-VALUES DISCRIMINANT (is protectedness reducible to important-AND-expensive? Python-only, parity stays 17/17) → completes R2 = R2a ∧ R2b ∧ R2c, the discriminant shape of R6 / H12

**What shipped.** `compute_r2c_discriminant(protected, card_sort)` in `analyze.py` — the R2 discriminant leg, widening R2b's *protected ≠ EXPENSIVE* (§17.3) to the **joint** *protected ≠ (important AND expensive)*. It regresses the **size** of a person's protected set `sacredness_i = |P_i|` (R2/§17.1 — the count of value slots they mark `never`) on TWO ordinary-value-strength channels drawn from TWO different logs — `importance_i` (§5.1 aspirational-self card-sort endorsement breadth, R6b's exact `importance`) and `mean_logprice_i` (mean log10 first-accept stake over that person's FINITE-priced cov cells) — and calls protectedness DISCRIMINABLE ⇔ **UPPER 95% bootstrap CI of the model R² < `R2C_R2_CEILING` (0.50)**. Two-sided: an INDEPENDENT `|P_i|` clears the ceiling → SUPPORTED (protectedness is a distinct construct — Tetlock 2003 / Baron & Spranca 1997: sacred values are quantitatively *insensitive*, not merely very costly); a REDUCIBLE `|P_i|` → NOT. Wired as R2 sub-key `R2c_discriminant`, fed by a new `--r2c-log` `{protected, card_sort}` shared-cohort bundle, threaded through `render_r2_result` (new `r2c` param + an "R2c DISCRIMINANT" reveal block). Seed `BOOTSTRAP_SEED + 38` (registry next-free; +37 was R1b-H10), new constants `R2C_R2_CEILING = 0.50` / `R2C_MIN_PARTICIPANTS = 8`. Reuses the R6b discriminant machinery verbatim (`_ols_r_squared`, `_bootstrap_ci_r2`, `_pearson_r`, `protected_value_sets`, `card_sort_scores`) — no new statistics.

**The §13.2-safe interpretive choice (PROPOSED, not auto-locked).** The design line "*regress `P_i`-membership on [inventory rank_v, log-price_v]*" reads literally as a per-`(person × value)` **cell LPM** — which collides with §13.2: a protected cell **has no finite `log-price_v`**, so the covariate is undefined exactly where the outcome is 1. The BUILT reading lifts the regression to the **person level** (mirroring R6b's per-person discriminant): the outcome is the protected-set **size** `|P_i|` (a §13.2-clean COUNT, never a price), and `log-price` enters as the person's mean over **FINITE cells only** — every protected `never` EXCLUDED, never finitized. So no `never` is ever coerced into a number: it counts *toward* `|P_i|` (the outcome) and *out of* `mean_logprice_i` (the predictor). This per-person lift is **proposed, not locked** — the per-cell LPM (with a cluster/participant bootstrap + a censoring-safe treatment of the undefined protected-cell price) stays a documented alternative for **Dave** to arbitrate at pre-registration (surfaced, not auto-decided).

**No manufactured trap (the honest two-sidedness).** `check_r2c_discriminant_lock()` (**7 assertions, all green**) builds N=40 synthetic cohorts through the REAL §17.1 + §5.1 pipelines, holds ONE `[importance, log-price]` predictor set **FIXED** (same card-sort → same importance; same finite-price *levels* → same mean log-price, unchanged by how many cells are `never` since every user keeps ≥2 finite cells), and flips the verdict **True→False** purely through the sacredness OUTCOME channel: INDEPENDENT (`|P_i| ⊥ [importance, log-price]` → SUPPORTED, R²≈0.002, upper CI 0.21 < 0.50), REDUCIBLE (`|P_i| = 3.0 + 2.2·z(importance) + 1.1·z(log-price) + noise`, clipped → NOT, R²≈0.95, upper CI 0.97 ≥ 0.50, `s_importance_r ≈ +0.86`). Crucially `|P_i|` is a **COUNT on the cov-`never` channel**, not an affine echo of the predictors (the H9b `cal_bias = stated − revealed` failure mode) — had such an identity existed the ⊥ draw would pin R² ≡ 1, yet here it is ~0, so the gate tracks the DATA. Plus: the exact-CI gate (`supported ⇔ ci_high < ceiling`) on both cohorts; the descriptive-leakage companion (`max|·r| < 0.20` independent, `|importance r| > 0.50` reducible); the < 8-participant floor → `None`; and a value-neutral render assertion.

**Fixture.** `analysis/fixtures/sample-r2c-log.json` — a 30-participant shared `{protected, card_sort}` cohort (keyed `r2c-u00..r2c-u29`, 10 cov slots per user) where `|P_i|` (0..8 `never`s) is drawn INDEPENDENT of both importance and log-price → known-ground-truth SUPPORTED (verified end-to-end: n=30, R²=0.0019, upper CI 0.254 < 0.50 → `supported: true`, `|P_i|·importance r ≈ 0`, `|P_i|·log-price r ≈ 0`). Every `never` carries `first_accept_stake: null` — never finitized (§13.2). Generated by a scratchpad generator (NOT committed); only the JSON is committed.

**Disciplines.** **Censoring (§13.2):** the crux — a protected `never` is the OUTCOME (counts into `|P_i|`) and is EXCLUDED from the price PREDICTOR, never finitized; `check_r2_censoring` gains a **5th assertion** proving `_r2c_mean_logprice_by_user` drops a `never` **even when it carries a spurious finite stake** (the protected-predicate, not just the null-stake, does the excluding) and drops an all-`never` user entirely (no mean, never priced 0). **Never pooled (§13.5):** `|P_i|` is a categorical set SIZE, never summed into a "sacredness score"; the two predictors stay separate columns; no pooled scalar. **Value-neutral / cohort-level (§17.5):** a large protected set is dual-read (integrity OR dogmatism), never ranked; the reveal is cohort-level ("protectedness NOT reducible to how important/expensive the values are"), never a per-person verdict. **N=1:** untouched — R2c is a cohort statistic (would mislead as an N=1 read), so it is Python-only with **no on-device reveal**; `poc-projection.js` is **UNTOUCHED** and **parity stays 17/17** — a single-repo (poc `main`) push. **NaN discipline (§1.5):** a NaN on any channel drops that user, never imputed.

**Docs.** scoring.md §17.4 flipped DEFERRED→BUILT (per-person formula + §13.2 exclusion + the PROPOSED per-person-vs-per-cell note) and the §17 Status line (R2 = R2a ∧ R2b ∧ R2c complete). build-and-validate.md item 4: header date (+"R2c discriminant 2026-07-10"), the "(a)/(c) deferred" split → "(a) 2026-07-10; (c) deferred", the (a) bullet rewritten from DEFERRED to a full BUILT callout.

**Gates.** `make check` green END-TO-END before commit: validate 66 scenarios + arcs; thresholds all ✓ (incl. the 7 new `r2c-discriminant` lock checks, the 5th `r2-censoring` assertion, and `R2` sub_met now `R2a ∧ R2b ∧ R2c_discriminant` with `reveal×12`); parity **17 passed, 0 failed** (unchanged — no reveal touched). Pushed: poc on `main` ONLY (runtime untouched).

**Operational note.** R2 is now complete for its buildable legs; only R2 (c) — the `taboo` marker's real collection + the cheap-talk / real-stakes validation (H-A2 → Phase-2) — remains, and it is **Dave-/runtime-gated**. Iteration 45's note had loosely bucketed "R2c" with the Dave-gated legs, but the R2c *discriminant* was in fact cheap-clean-buildable synthetically (exactly like R6b / H10b), so this iteration built it. **The discriminant/dampening backlog for the BUILT H9–R6 family is now essentially exhausted:** the only remaining leg is R1b **H11** (parochialism `β_i`, §16), which is **value-charged → Dave-gated** (building it takes Singer's side against H11's own Singer↔Williams value-neutrality). The next tier of genuinely-buildable-without-Dave work is the **spec-only NEW branches** — **R3** (moral disengagement, `r3-moral-disengagement.md`), **R4** (moral attentiveness, `r4-moral-attentiveness.md`), **R5** (moral typecasting, `r5-moral-typecasting.md`) — all design-complete but not yet in `analyze.py` (confirmed: no `compute_r3/r4/r5`); each is a **full new branch** (new primitive + fixture + lock), larger than a discriminant leg but self-contained and synthetic. Everything else is Dave-/κ-/runtime-gated: R6c meta-gap + A3 language legs (human-κ), A4 answer-revision capture (runtime), all real-collection phrasing locks, and every pre-registration DECISIONS lock (propose, never auto-lock). **Next fire: scope the cheapest-clean of R3/R4/R5 and build its reliability + one discriminant/directional leg; if none is cleanly scoped without Dave, say so and stop (no filler).**

---

## Iteration 45 — 2026-07-10 — R1 · R1b H10 DAMPENING LEG (does internalization dampen the observer effect? Python-only, parity stays 17/17) → the SECOND of R1b's three H10–H12 dampening legs

**What shipped.** Iteration 44's operational note flagged two remaining non-Dave increments — the R1b **H10** and **H11** dampening legs — and picked the **cheapest-clean** one: H10. `compute_r1b_h10_dampening(context, identity, tag_map)` in `analyze.py` asks whether a more internalized moral identity **dampens the H10 observer effect** `O_i` — the per-person public−anonymous concern shift (§15, the read the H10c cohort anchor aggregates) — not only the §6 over-claim gap (R1b gap leg) and the §18 self–other asymmetry (R1b H12 leg). Regress each person's signed observer gap `O_i = observer_gap_by_user(context_item_records(context, tag_map))[i]` (§15, `mean(public score) − mean(anonymous score)`) on their `internalization_i = centrality_facet_by_user(identity, "internalization")` (§19.1) across a **shared** cohort; `r = corr(internalization_i, O_i)`; **supported ⇔ UPPER 95% bootstrap CI of `r` < 0** (one-sided). Wired as a SEPARATE R1 key `R1b_h10_dampening` (alongside `R1a`, `R1a_symbolization`, `R1c`, `R1b_moderation`, `R1b_h12_dampening`), fed by a new `--r1b-h10-log` `{identity, context}` shared-cohort bundle, threaded through `render_r1_result` (new 9th param + a companion "R1b H10-DAMPENING" line). Seed `BOOTSTRAP_SEED + 37` (registry next-free; now +38), reusing `R1B_MIN_PARTICIPANTS = 8` + `R1B_MODERATION_CEILING = 0.0` (the R1b family constants).

**The one net-new primitive (and why H10, not H11).** `observer_gap_by_user(records)` was **factored out of** `compute_h10c_observer_effect` (which already computed the same per-user gap inline) — the H10c anchor now calls it, output bit-identical (same `sorted(set(pub) & set(anon))` ordering, same `<3` cohort floor). The leg then composes exactly like the R1b gap leg: it takes RAW context entries + `tag_map` and parses `O_i` internally via the same parity-locked §2 `item_score` the revealed pipeline uses. **H10 over H11 because H10 is value-neutral-dampenable and H11 is not:** the observer effect (public > anonymous) is a well-established self-presentation/reactivity distortion that H10c already treats as *the* observer effect, and Aquino & Reed 2002 specifically predict the internalization/private-regard facet → **less** observation-dependence. `O_i` stays **dual-read** (large = norm-responsiveness OR performativity; small/negative = authentic consistency OR context-insensitivity), cohort-level, never per-person. The **H11 leg is DEFERRED to Dave** because calling parochialism (`β_i`, §16) a "bias to dampen" would take Singer's impartialist side against H11's own explicit Singer↔Williams/MacIntyre value-neutrality (a smuggled value, CV-2).

**No manufactured trap (the honest two-sidedness).** The two channels are **INDEPENDENT** — internalization rides the identity log; `O_i` rides the context log's public/anonymous items — so no algebraic identity forces the slope. `check_r1b_h10_dampening_lock()` (**7 assertions, all green**) builds N=40 synthetic cohorts through the REAL §19.1 + §2-item_score-→-§15-observer-gap pipelines, holds ONE internalization profile **FIXED**, and flips the verdict **True→False→False** purely through the O channel: NEGATIVE (`O = −z(internalization) + noise` → SUPPORTED, `r ≈ −0.90`, upper CI < 0), NULL (`O ⊥ internalization` → NOT, CI straddles 0, `r ≈ −0.01`), POSITIVE (`O = +z(internalization) + noise` → NOT, `ci_low > 0`, `r ≈ +0.85` — the WRONG direction is one-sided rejected). Plus: the exact-CI gate (`supported ⇔ ci_high < ceiling`) on all three cohorts; the < 8-participant inclusion floor → `None`; and a value-neutral render assertion. Cohort seeds (`2 / 3 / 2`) were found under the +37 bootstrap by a scratchpad seed-search (never committed); the O channel is realized by quantizing each target onto achievable honesty-tag item scores (anonymous pole fixed at mean 0.0).

**Fixture.** `analysis/fixtures/sample-r1b-h10-log.json` — 24-participant shared `{identity, context}` cohort (identity reused verbatim from the H12 fixture; context newly built) where internalization NEGATIVELY predicts `O_i` (SUPPORTED; verified end-to-end through the real analyzer: `r = −0.932`, 95% CI [−0.963, −0.883], `ci_high < 0` → `supported: true`). Context items are `truth-telling` honesty-axis choices tagged `context:public` / `context:anonymous` (pure stratifiers, zero score contribution); generated by a scratchpad generator (NOT committed); only the JSON is committed.

**Disciplines.** **Value-neutral / cohort-level (§15 / §19.4):** never a per-person verdict — `O_i` is dual-read (no observer pole scored "better"), neither internalization pole ranked; the render mirrors the already-blessed R1b framing (preserves the `check_r1b_moderation_lock` substrings). **Never pooled (§13.5):** `O_i` and internalization stay SEPARATE keys; no pooled scalar (the lock's `POOLED` set-scan asserts it). **Censoring (§13.2):** N/A — `O_i` is a signed score gap, not a censored price; the `never`/|8.0| lock is untouched. **Parity:** this leg is **Python-only, no on-device reveal** (cohort statistic — would mislead as an N=1 read, exactly like R1b_moderation / H12-dampening stay off-device), so `poc-projection.js` is **UNTOUCHED** and **parity stays 17/17** — a single-repo (poc `main`) push. **NaN discipline (§1.5):** a NaN on either channel drops that user, never imputed. **H10c unchanged:** the refactor preserved H10c's output bit-for-bit (its own `<3` floor + seed +14 intact).

**Docs.** scoring.md §19.5 header + Status flipped to include the H10 dampening leg BUILT (formula + independence + dual-read note); the old "DEFERRED — the H10 / H11 dampening legs" block split into a BUILT H10 subsection + a residual "DEFERRED — the H11 dampening leg" (with the value-charged rationale); §19.6 deferred-legs line updated. build-and-validate.md item 6: header date list (+"R1b H10 dampening leg 2026-07-10"), a BUILT callout, Deferred (a) trimmed H10/H11 → H11-only (Dave-gated, value-charged).

**Gates.** `make check` green END-TO-END before commit: validate 66 scenarios + arcs; thresholds all ✓ (incl. the 7 new `r1b-h10-dampening` lock checks + the `check_r1` re-derivation); parity **17 passed, 0 failed** (unchanged — no reveal touched). Pushed: poc on `main` ONLY (runtime untouched).

**Operational note.** ONE documented non-Dave leg remains and it is **value-charged**: the R1b **H11** dampening leg (parochialism `β_i`, §16) — building it would take Singer's side against H11's Singer↔Williams value-neutrality, so it is **Dave-gated**, not auto-buildable. Everything else documented is Dave-/κ-/runtime-gated: A4 answer-revision capture (§4 Q1 runtime), R2c / R6c / A3 cohort-coupled + human-rater legs, all real-collection phrasing locks, and every pre-registration DECISIONS lock (propose, never auto-lock). The R1b dampening family is now **2 of 3 built** (gap + H12 + H10); H11 is the only one left and it needs Dave. **Next fire: if no leg is cleanly scoped without Dave, say so in the journal and stop (no filler) — surface the H11 value-neutrality decision to Dave rather than guessing.**

---

## Iteration 44 — 2026-07-10 — R1 · R1b H12 DAMPENING LEG (does internalization dampen the holier-than-thou asymmetry? Python-only, parity stays 17/17) → the FIRST of R1b's three H10–H12 dampening legs

**What shipped.** The item flagged as the last documented non-Dave increment (Iter 43's operational note: "(a) the R1b H10–H12 dampening legs"), scoped to its **cheapest-clean** leg. `compute_r1b_h12_dampening(hypocrisy, identity)` in `analyze.py` asks whether a more internalized moral identity **dampens the H12 self–other severity asymmetry** `H_i` (§18.1, holier-than-thou), not only the §6 over-claim gap the existing R1b gap leg covers. Regress each person's signed asymmetry `H_i = hypocrisy_asymmetry_by_user(hypocrisy)[i]` (§18.1) on their `internalization_i = centrality_facet_by_user(identity, "internalization")` (§19.1) across a **shared** cohort; `r = corr(internalization_i, H_i)`; **supported ⇔ UPPER 95% bootstrap CI of `r` < 0** (one-sided). Both inputs were already built and parity-locked — this leg is pure composition + a new directional gate. Wired as a SEPARATE R1 key `R1b_h12_dampening` (alongside `R1a`, `R1a_symbolization`, `R1c`, `R1b_moderation`), fed by a new `--r1b-h12-log` `{identity, hypocrisy}` shared-cohort bundle, threaded through `render_r1_result` (new 8th param + a companion "R1b H12-DAMPENING" line). Seed `BOOTSTRAP_SEED + 36` (registry next-free was +36; now +37), reusing `R1B_MIN_PARTICIPANTS = 8` + `R1B_MODERATION_CEILING = 0.0` (the R1b family constants).

**Why it is not double-counting H12b.** H12b (§18.5) regresses `H_i` on the person's **OWN** `[gap, cal_error]` on an **R²-ceiling** (is hypocrisy *reducible* to over-claiming + self-insight?). This leg regresses `H_i` on **internalization** (a DIFFERENT predictor, from a different channel) on a **signed-slope** gate (does moral identity *moderate* hypocrisy?). Different predictor, different question — moderation, not reducibility. It is the exact H12 analog of the already-Dave-reviewed R1b **gap** leg (`compute_r1b_moderation`), only the outcome channel swapped from the §6 gap to the §18 asymmetry.

**No manufactured trap (the honest two-sidedness).** The two channels are **INDEPENDENT** — internalization rides the identity log; `H_i` rides the matched-severity hypocrisy log — so no algebraic identity forces the slope (unlike H9b's signed `cal_bias = stated − revealed` echo). `check_r1b_h12_dampening_lock()` (**7 assertions, all green**) builds N=40 synthetic cohorts through the REAL §19.1 + §18.1 pipelines, holds ONE internalization profile **FIXED**, and flips the verdict **True→False→False** purely through the H channel: NEGATIVE (`H = −z(internalization) + noise` → SUPPORTED, `r ≈ −0.85`, upper CI < 0), NULL (`H ⊥ internalization` → NOT, CI straddles 0, `r ≈ +0.07`), POSITIVE (`H = +z(internalization) + noise` → NOT, `ci_low > 0`, `r ≈ +0.85` — the WRONG direction is one-sided rejected). Plus: the exact-CI gate (`supported ⇔ ci_high < ceiling`) on all three cohorts; the < 8-participant inclusion floor → `None`; and a value-neutral render assertion. Had an algebraic identity existed the ⊥ NULL cohort would pin `|r| ≡ 1`; here it is ~0 — the gate tracks the DATA. Cohort seeds reuse the moderation lock's (`10015 / 200003 / 400053`), confirmed robust under the +36 bootstrap by a scratchpad seed-search (never committed).

**Fixture.** `analysis/fixtures/sample-r1b-h12-log.json` — 24-participant shared `{identity, hypocrisy}` cohort where internalization NEGATIVELY predicts `H_i` (SUPPORTED; verified end-to-end through the real analyzer: `r = −0.895`, 95% CI [−0.947, −0.808], `ci_high < 0` → `supported: true`). Generated by a scratchpad generator (NOT committed); only the JSON is committed.

**Disciplines.** **Value-neutral / cohort-level (§18.4 / §19.4):** never a per-person verdict — a very negative `H_i` is **harsher-on-self** (described, never ranked "better"), and neither internalization pole is ranked; the render mirrors the already-blessed R1b_moderation framing. **Never pooled (§13.5):** `H_i` and internalization stay SEPARATE keys; no pooled scalar (the lock's `POOLED` set-scan asserts it). **Censoring (§13.2):** N/A — `H_i` is a signed severity delta, not a censored price; the `never`/|8.0| lock is untouched. **Parity:** this leg is **Python-only, no on-device reveal** (cohort statistic — would mislead as an N=1 read, exactly like R1b_moderation / H12b stay off-device), so `poc-projection.js` is **UNTOUCHED** and **parity stays 17/17** — a single-repo (poc `main`) push. **NaN discipline (§1.5):** a NaN on either channel drops that user, never imputed.

**Docs.** scoring.md §19.5 header + Status flipped to include the H12 dampening leg BUILT (formula + independence + DISTINCT-from-H12b note); the old "DEFERRED — the H10–H12 dampening legs" block split into a BUILT H12 subsection + a residual "DEFERRED — the H10 / H11 dampening legs". build-and-validate.md item 6: header date list (+"R1b H12 dampening leg 2026-07-10"), a BUILT callout, Deferred (a) trimmed H10–H12 → H10/H11.

**Gates.** `make check` green END-TO-END before commit: validate 66 scenarios + arcs; thresholds all ✓ (incl. the 7 new `r1b-h12-dampening` lock checks + the `check_r1` re-derivation); parity **17 passed, 0 failed** (unchanged — no reveal touched). Pushed: poc on `main` ONLY (runtime untouched).

**Operational note.** Two documented non-Dave increments remain, both larger than a cheap-clean leg: **the R1b H10 and H11 dampening legs** (H10 framing / H11 anchor asymmetries) — each needs a new per-person asymmetry extraction coupled to its own §15/§16 pipeline (H10 needs per-person observer-gap; H11 is parochialism, value-charged). Everything else documented is Dave-/κ-/runtime-gated: A4 answer-revision capture (§4 Q1 runtime), R2c / R6c / A3 cohort-coupled + human-rater legs, all real-collection phrasing locks, and every pre-registration DECISIONS lock (propose, never auto-lock). Next fire: build an H10 or H11 dampening leg if cleanly scoped, else say so and stop (no filler).

---

## Iteration 43 — 2026-07-10 — A4 · On-device DECISION-CONFLICT REVEAL (conflictReads in poc-projection.js; N=1, parity 16→17) → the 8th and FINAL on-device reveal — the reveal program is COMPLETE

**What shipped.** The A4 on-device `conflict(i, domain)` reveal — the item Iteration 42's operational note flagged as one of the two remaining buildable-without-Dave increments ("needs a NEW on-device MULTIPLE-OLS residualizer… a real port with parity risk"). `conflictReads(records)` in `poc-projection.js` computes the §22.1 quantity on-device for one user, mirroring the analyzer's `conflict_by_user_domain`: person i's **within-person relative effort per domain** = the mean over their domain-d items of `rt_z(i, item)`, where `rt_z` = the within-person z-score of `response_time_ms` **residualized on `[prompt_chars, presented_position]`** (OLS), timeouts + the TIMED quick-fire set (CV-1) excluded. The net-new work was a faithful **port** of known Python logic (no new scoring, no new fixture): `_solveLinear` (partial-pivot Gaussian elimination, **first-max tie-break** to match Python's `max`, `< 1e-12` singular guard → `null`), `_olsResiduals` (normal equations XᵀXβ = Xᵀy; `null` when underdetermined / rank-deficient → caller falls back to within-person **mean-centering**), the A4 accessors (`_a4User`/`_a4Item`/`_a4Excluded`/`_a4PromptChars`), the `A4_MIN_ITEMS = 4` floor, the **population** SD (÷ N, NOT the module's sample `sd()`), and the `sd == 0 → 0.0` degenerate branch — all matched exactly. Added to the module exports + `_constants`.

**Parity (16→17).** New `check_impl_parity.py` case: JS == Python on all **8 fixture participants** (`sample-process-log.json`, reused as-is) **plus 3 synthetic edge users appended post-load** that pin the discrete branches a float tolerance can't catch: `zz-below-floor` (3 < 4 scorable items → below `A4_MIN_ITEMS` → no cells → Python-absent ⇔ JS `ok:false`), `zz-rankdef` (constant `presented_position` → position collinear with intercept → rank-deficient → `_ols_residuals` null → mean-centering **fallback with SD>0**), and `zz-flat` (constant `prompt_chars` → rank-deficient AND constant `response_time_ms` → residuals exactly 0 → **`sd==0` → conflict 0.0**). All 4 discrete branches now locked (the 8 clean users exercise the OLS-full-rank path). Gate line: `conflictReads: JS == Python on all 11 participants`.

**De-risking (the "parity risk" Iter 42 flagged, resolved).** Before writing the port I confirmed the parity gate compares floats with **`abs(a−b) < 1e-9` TOLERANCE**, not bit-exactness — so a faithful port of the same float64 arithmetic passes trivially; the only genuine risk is DISCRETE branches (rank-deficiency, `sd==0`, item floor) diverging, which the 3 edge users control by construction. A scratchpad bite-test (never committed) measured the actual worst per-domain divergence across all 11 users at **7.77e-16** — six orders of magnitude under the gate — and separately confirmed each edge user takes its INTENDED branch (`zz-rankdef` → fallback sd=223.6; `zz-flat` → fallback sd=0.0 exact; `u01` → OLS-full-rank). Even the `x**0.5` (Python) vs `Math.sqrt` (JS) seam is a non-issue: with SDs in the hundreds-of-ms range a 1-ULP sd difference propagates to ~1e-14 in the z, far under 1e-9.

**Why the N=1 reveal is legitimate.** The within-person z-score and per-domain mean **never touch another person's records** — every step is scoped to person i alone. A pre-build probe verified this exactly: the cohort-filtered analyzer value for u01 equals the value computed from ONLY u01's records, to 1e-15. So the on-device read needs no cohort standardization. A4a's per-domain **reliability** and A4b's cohort **discriminant** correctly stay OFF-device (they *are* cohort statistics, not per-person, and would mislead as an N=1 verdict); only `conflict(i, domain)` crosses to the device.

**Disciplines.** **Value-neutral (§3/§22.3), EXTRA force:** conflict is **EFFORT/ambivalence, described** — never a moral-framework label (slow ≠ deontological; Bago & De Neys 2019), never a virtue ranking (effortful virtue is not worse than easy virtue), and the reveal carries the RT-only/exploratory caveat. **Never pooled (§13.5):** the reveal is per-`(domain)` for one person, never a summed cross-person "conflict score." **Censoring (§13.2):** N/A to A4 (RT is not a censored price), untouched. **Parity:** this branch touches the on-device reveal, so it changes **both** `analyze.py` (comment status) and `poc-projection.js` (the port) together, under the parity lock. **DETERMINISTIC** — no bootstrap, **no seed consumed** (registry next-free stays `+36`). Synthetic data only.

**Docs.** scoring.md §22: the §22 intro analyzer-comment "DEFERRED" note flipped to "ON-DEVICE REVEAL SHIPPED"; §22.4 heading + lead-in + the reveal bullet flipped DEFERRED→SHIPPED with parity details; new **§22.6 "The on-device conflict reveal (SHIPPED 2026-07-10)"** (the `conflictReads` port, the N=1 self-containment justification, the 3-edge-user parity lock, value-neutrality, 8th/final reveal). build-and-validate.md item 9: header (+"on-device conflict reveal 2026-07-10"), the deferred (b) bullet flipped to a SHIPPED callout (only revision-capture remains deferred). `analyze.py` A4 header comment updated to point at `conflictReads` + the parity lock.

**Gates.** `make check` green END-TO-END before commit: validate 66 scenarios + arcs; thresholds all ✓; parity **17 passed, 0 failed** (`conflictReads` exercised, confirmed in the gate line). Pushed: poc on `main` AND runtime on `master` (this branch touches the reveal → both repos; staged ONLY `patterns-of-choice/runtime/poc-projection.js` in daliu.github.io, never the unrelated dirty `resume/regenerate.sh`).

**Operational note — the on-device reveal program is COMPLETE.** All 8 per-person reveals for the H9–R6 / A-series family now ship on-device and are JS↔Python parity-locked: R1 facets (Iter 35), R6 claim-types (36), H12 hypocrisy (37), H10 context-variability (38), H11 circle-shape (39), R2 protected-set (40), H9 self-calibration (41), and now A4 conflict (43). Parity gate: 17 reveals/cases. **Buildable-without-Dave work is now very thin.** The last documented non-Dave increment is **(a) the R1b H10–H12 dampening legs** — whether internalization *also* blunts the framing (H10) / anchor (H11) / decoy (H12) asymmetries; each adds two more cohort channels + a new multi-channel {session, card_sort, identity, +H10/H11/H12} fixture and couples to its own §15/§16/§18 pipeline (larger than a cheap-clean increment, but buildable). Everything else documented is Dave-/κ-/runtime-gated: A4 answer-revision capture (§4 Q1 runtime), R2c / R6c / A3 cohort-coupled + human-rater legs, all real-collection phrasing locks, and every pre-registration DECISIONS lock (propose, never auto-lock). Next fire: build the R1b dampening legs if cleanly scoped, else say so and stop (no filler).

---

## Iteration 42 — 2026-07-08 — R1 · SYMBOLIZATION-facet split-half reliability (R1a_symbolization; Python-only, parity stays 16/16) → R1's reliability story now covers BOTH facets

**What shipped.** Backlog item 6(b): the deferred symbolization-facet reliability leg — R1a was anchored on internalization only; this completes the pair. `compute_r1a_symbolization_reliability(records)` in `analyze.py` mirrors `compute_r1a_reliability` **exactly** (`_odd_even_sessions` → `centrality_facet_by_user(…, "symbolization", …)` on each half → `_pearson_r` → `_bootstrap_ci_r`; supported ⇔ lower 95% CI ≥ `R1A_RELIABILITY_FLOOR` = 0.40) but on the public/symbolization dimension, seed `BOOTSTRAP_SEED + 35`. Wired into the R1 JSON block as a SEPARATE key `R1a_symbolization` (alongside `R1a`, `R1c`, `R1b_moderation`) and threaded through `render_r1_result` (a new companion human-readable line "corr of symbolization"). The two facets' reliabilities are reported **side by side and never pooled** into one figure (§13.5), exactly as the facet means are (§19.1). No new fixture — reuses `sample-identity-centrality-log.json` as-is; **no runtime touch** (`poc-projection.js` untouched), so this is Python-only and parity stays 16/16.

**Ground truth (probed before building).** A throwaway scratchpad probe (never committed) measured the existing fixture's symbolization test–retest before I committed to the branch: `n_shared=12, r=1.0, ci=(1.0, 1.0)`, clears the 0.40 floor → `met=True` — the fixture's per-person responses are constant across sessions by construction, so both facets are perfectly reliable and no fixture perturbation was needed. De-risked the pick (symbolization's signal is emergent, not engineered, so it could have failed the bar and forced an ugly met=False lock or a risky edit to the load-bearing fixture — it didn't).

**Gate — and a rigor upgrade.** `check_r1` gains the `R1a_symbolization` sub-expectation (`met=True`) via the generic `sub_met` loop. Beyond mirroring, I closed a latent gap: the reliability legs were gated only by a **stamped** `pre_registered_threshold_met` flag (the generic loop trusts it), unlike `R1b_moderation`, which re-derives its verdict from the shipped CI. `check_r1` now **re-derives BOTH reliability legs' `met` against their own CI** — `met ⇔ (ci_low ≥ threshold_low)`, with the degenerate `ci_low=None` (NaN→None via `_h9_json`) ⇒ `met=None` case handled — so a stamped flag alone can no longer satisfy the gate, in either direction. A scratchpad bite-test (never committed, 4 assertions) confirms it: honest payload passes; a leg stamping `met=True` with `ci_low=0.10 < 0.40` is REJECTED ("the reliability gate must equal the arithmetic"); a leg stamping `met=False` with `ci_low=0.9 ≥ 0.40` is REJECTED (both directions bite); the None/None degenerate is not tripped by the arithmetic. Gate line now `R1: ✓ R1a=True, R1a_symbolization=True, R1c=True, R1b_moderation=True, reveal×12, profile×12`.

**Disciplines.** The load-bearing one is **no-pool (§13.5)**: symbolization reliability is its OWN leg, never averaged with internalization into a single "centrality reliability" — the reliability figures stay as separate as the facet means. **Same bar, genuine test**: symbolization is the more situational, outward-display dimension (Aquino & Reed 2002), so clearing the identical 0.40 floor is informative, not a foregone conclusion. **Value-neutral**: neither facet's reliability is ranked above the other; this is descriptive construct validation, not a verdict. Deterministic percentile bootstrap, pre-committed seed `+35` (registry: `+35 → R1a_symbolization`, next-free now `+36`). Synthetic data only; no reveal, no on-device surface, N=1 disciplines untouched.

**Docs.** scoring.md §19: §19.2 heading broadened to "facet reliability: internalization AND symbolization (BUILT)" + a both-facets paragraph (separate legs, seed +35, the CI re-derivation), §19 Status line (R1a now both facets), §19.6 "no cohort statistics on-device" bullet (both `R1a` + `R1a_symbolization` stay off-device). build-and-validate.md item 6: headline (+"R1a symbolization-facet reliability 2026-07-08", "R1 core = R1a (both facets) ∧ R1c ∧ R1b"), a BUILT callout, and the deferred list flipped ((b) symbolization dropped → BUILT; old (c) real-collection re-lettered to (b)).

**Gates.** `make check` green END-TO-END before commit: validate 66 scenarios + arcs; thresholds all ✓ (`R1a_symbolization=True` exercised, confirmed in the gate line, not silently skipped); parity **16 passed, 0 failed** (unchanged — Python-only). Pushed: poc on `main` only (no runtime change this iteration).

**Operational note.** R1's reliability story is now COMPLETE (both facets tested, separately). Buildable-without-Dave work is thinning: the two documented next-increments that don't need Dave are (a) the **A4 on-device `conflict(i, domain)` reveal** — needs a NEW on-device MULTIPLE-OLS residualizer (RT on `[prompt_chars, presented_position]`; existing `olsSlope` is single-predictor only), a real port with parity risk, and (b) the **R1b H10–H12 dampening legs** — each adds two more cohort channels + a new multi-channel fixture. Both are larger than the recent cheap-clean increments. R2c-discriminant remains RULED OUT autonomously (a protected slot is a right-censored `never` with no finite log-price; regressing membership on `[rank, log-price]` without finitizing the censored tail is a pre-registration design decision → surface to Dave, don't guess). Next fire picks the cheapest of (a)/(b), or if neither is cleanly buildable, says so and stops (no filler).

---

## Iteration 41 — 2026-07-08 — H9 · On-device SELF-PREDICTION CALIBRATION REVEAL (selfCalibration in poc-projection.js; N=1, parity 15→16) → the SEVENTH shipped on-device reveal for the H9–R6 family

**What shipped.** Backlog item 1(b): H9's deferred on-device reveal, ported after R1 (Iter 35), R6 (Iter 36), H12 (Iter 37), H10 (Iter 38), H11 (Iter 39), and R2 (Iter 40) under the same pattern — the LAST cheap-clean reveal port (A4's `conflict(i, domain)` needs on-device multiple-OLS residualization, deferred as harder). `selfCalibration(records, tagMap)` in `poc-projection.js` (§14.8) computes the §14.2 N=1 reveal on-device from resolved prediction↔choice pairs `{channel, domain, predicted_tags, realized_tags}`: over admitted **axis-channel** probes, score both sides with the SAME parity-locked `itemScore` (`e = pred − rev`), then per person the signed `cal_bias` = mean(e) (self-enhancement direction) + magnitude `cal_error` = mean(|e|) (self-knowledge; lower = better) + `n_probes`. The cost-of-virtue PRICE channel is never pooled in (§14.7) and carries its own §14.1 `never`-censoring — intentionally absent. `ok: false` exactly when no admitted axis probe exists ⇔ analyzer-absent. Python side: NO refactor — `calibration_person_indices(calibration_axis_records(...))` was already the per-user census; `analyze.py --predictions` now emits `H9.calibration_reveal` (per-user `{user, cal_bias, cal_error, n_probes}`, user-sorted) alongside the pre-existing `person_indices_n` count.

**Parity (15→16).** New `check_impl_parity.py` case: JS == Python on all **16 participants** — the 12 fixture users (`sample-predictions.json`, reused as-is via `load_predictions`; cov records filtered by channel on both sides) plus 4 post-load edge users: `zz-cov-only` (cost-of-virtue channel only → no axis probe → Python-absent ⇔ JS `ok: false`), `zz-na-tags` (an axis probe whose predicted tag is off-primary-axis → `pn == 0` NA → excluded on both sides), `zz-mixed` (one admitted probe + one NA probe → `n_probes == 1`), and `zz-neg` (predicted LESS virtuous than acted → `e < 0` → **negative `cal_bias`**, proving JS carries the SIGNED bias, never abs/clamps it). Both the NA gate and the signed-mean path exercised live. tagMap bridged to node via `require(tag-axis-map.v0.1.json)`, the same map the `itemScore` parity case already locks.

**Shape-lock.** `check_h9` now shape-locks the emitted reveal: every row is **EXACTLY** `{user, cal_bias, cal_error, n_probes}` (exact-key-match — strictly stronger than a banned-list: it admits no `calibration_score`/`self_knowledge_score`/`accuracy`/`rank`/`percentile`/`verdict`/`grade`/`cross_person` key at all, §14.7); `cal_bias`/`cal_error` finite floats; `cal_error >= 0`; the arithmetic invariant **`cal_error >= |cal_bias|`** (mean|e| ≥ |mean e|, Jensen — the H9 analog of R2's `n_professed ≤ n_slots_probed`); `n_probes` a positive non-bool int; and `len(reveal) == person_indices_n` count-coherence. Bite-tested from the scratchpad (never committed): 16/16 — clean reveal + absent-reveal pass; four banned keys, Jensen violation, negative error, NaN/inf, zero/bool/float `n_probes`, missing key, empty list, and count mismatch all rejected. Gate line now `H9: ✓ H9a=True, H9b_stability=True, H9b_discriminant=True, H9c=True, cov[finite=2,censored=3], reveal×12`.

**Disciplines.** The load-bearing one is **no cross-channel pooling (§14.7)**: only `channel === "axis"` feeds the reveal — the CoV price channel with its §14.1 categorical `never` censoring never enters, so no axis/log-dollar pseudo-quantity is manufactured. **N=1 interpretability**: `pred` and `rev` share one pre-defined axis, so NO cohort standardization (contrast the §6 gap) — reveal-eligible for a single user, rendered descriptively ("you predict yourself as more generous than you act, by about X, across N probes"), never a score-out-of-N. **Value-neutral both directions**: `cal_bias` is signed — over-confidence AND under-confidence (a very negative bias = modesty) both described, never ranked; a lower `cal_error` (better self-knowledge) is never cross-person-ranked in any user-facing surface (the cohort H9a/b/c stats stay research-side). **NA gate exact**: an item with no primary-axis contribution is excluded identically on both sides (`pred.n === 0 || rev.n === 0` ⇔ `pn == 0 or rn == 0`). Deterministic — no bootstrap, no seed consumed; registry next-free stays +35. Synthetic data only.

**Docs.** scoring.md §14: §14.2 status flipped (reveal BUILT 2026-07-08, seventh in the family; H9b discriminant stays cohort-level), new §14.8 (shape + exact-mirror/axis-only/censoring bullet, no-composite/no-cross-person-rank bullet, value-neutral-both-directions + Jensen-invariant bullet). build-and-validate.md item 1: header + "(a) 2026-07-01, (b) 2026-07-08; (c) deferred" preamble + (b) body flipped to BUILT.

**Gates.** `make check` green END-TO-END before commit: validate 66 scenarios + arcs; thresholds all ✓ (H9 line gains `reveal×12`); parity **16 passed, 0 failed** ("selfCalibration: JS == Python on all 16 participants"). Both repos pushed: poc on `main`, runtime on `master` (only `poc-projection.js` staged; unrelated dirty `resume/regenerate.sh` untouched).

**Operational note.** Seven on-device N=1 reveals now shipped for the H9–R6 family (R1/R6/H12/H10/H11/R2/H9). The ONLY remaining reveal port is **A4 `conflict(i, domain)`** — deferred as harder: it needs an on-device MULTIPLE-OLS residualizer (RT on `[prompt_chars, presented_position]`), whereas the existing `olsSlope` is single-predictor only. The next fire either builds that residualizer or, if no cheap-clean buildable-without-Dave branch remains, says so in the journal and stops (no filler).
