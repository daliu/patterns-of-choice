...
- *H9 self-prediction calibration analysis (added 2026-06-08 per DECISIONS §19):* Before a designated calibration probe resolves, the participant records a non-binding **prediction** of their own choice on the same response surface (the *prediction beat* — a cost-of-virtue ladder rung or a paired-probe option). For participant *i* on probe *p*: `e_i^p = pred_i^p − rev_i^p` (signed calibration error, predicted minus revealed on the [−1,+1] primary axis, or predicted−actual break-point for cost-of-virtue); `cal_bias_i = mean_p e_i^p`; `cal_error_i = mean_p |e_i^p|`. Both are within-person and need no cross-scale/cross-person standardization (`h9-self-calibration.md` §1.5). **H9a (self-enhancement bias):** lower 95% bootstrap CI of `mean_i cal_bias_i` ≥ 0.10, over the three consensual-pole domains only (the in-group axis lacks a consensual desirable pole and is excluded from the signed test, exploratory only). **H9b (distinct, stable axis):** split-window test-retest of `cal_error_i` lower 95% CI ≥ 0.40 *and* discriminant — in a regression of `cal_error_i` on [`gap_i`, `revealed_level_i`], model R² upper 95% CI < 0.50. **H9c (stakes-blindness):** `blind_i = mean|e_i^p|` over the high-stakes (H8b) pool − `mean|e_i^p|` over the low-stakes (H8a) pool, axis-scale pools only; lower 95% bootstrap CI of `mean_i blind_i` > 0 (one-sided, directional). Cost-of-virtue break-point calibration (log-dollar units, censoring-aware) is reported convergently, never pooled into `blind_i` (`scoring.md` §14.5/§14.7). All bootstrapped per `scoring.md` §8 with the pre-committed seed. **Reactivity control (mandatory):** a counterbalanced subset of comparable probes receives no prediction beat; revealed behavior on predicted vs non-predicted items is contrasted to estimate and net out the question–behavior (mere-measurement) effect before H9 scoring (`scoring.md` §14.6). Full operationalization in `scoring.md` §14; censoring discipline inherited from §13.2/§13.3 (a `never` endpoint suppresses the signed error — never made finite).
...
- *Secondary (added 2026-06-08):* H9a self-enhancement prediction bias — lower 95% CI of `mean_i cal_bias_i` ≥ 0.10 (consensual-pole domains).
- *Secondary (added 2026-06-08):* H9b calibration distinct + stable — `cal_error_i` test-retest lower 95% CI ≥ 0.40 AND discriminant-regression R² upper 95% CI < 0.50.
- *Secondary (added 2026-06-08):* H9c stakes-blindness — lower 95% CI of `mean_i blind_i` > 0 (one-sided).
...
| H9a | Participants over-predict their own value-alignment: mean signed self-prediction error positive, lower 95% CI of `mean_i cal_bias_i` ≥ 0.10 | Epley & Dunning 2000 self–other prediction asymmetry; see `h9-self-calibration.md` and DECISIONS §19 |
| H9b | Self-prediction calibration is a distinct, stable axis: `cal_error_i` test-retest lower 95% CI ≥ 0.40 and discriminant-regression R² upper 95% CI < 0.50 | Vazire 2010 SOKA; Tetlock calibration; see `h9-self-calibration.md` |
| H9c | Self-prediction error is larger at high stakes than at low stakes: lower 95% CI of `mean_i blind_i` > 0 | Loewenstein hot–cold empathy gap; `validity-threats.md` EV-4; see `h9-self-calibration.md` |
...
**H9 status.** Added 2026-06-08 per DECISIONS §19. Secondary (reported with effect sizes/CIs; never a gate-criterion — the H1–H3 primary gate is unchanged). Tests the three sub-hypotheses above via a counterbalanced **prediction beat** on existing cost-of-virtue and H8 paired probes — no new scenarios (DECISIONS §19; the §16/§17 corpus is untouched) — with operationalization in `scoring.md` §14 and full rationale in `h9-self-calibration.md`. The combined H9 claim — people over-predict their virtue, this calibration is a stable trait distinct from the gap, and it fails worst at high stakes — requires H9a ∧ H9b ∧ H9c. **Implementation note:** H9 is not yet computed in `analyze.py`; it awaits the `prediction` event type (`types.ts` `PredictionLogEntry`, DECISIONS §19 pending), after which the statistics reuse the existing bootstrap machinery.
...
**H9 falsification.** H9 as a whole is falsified unless all three sub-hypotheses pass. Partial results are reported and published — e.g. "people over-predict their virtue but calibration is not separable from the gap" (H9a∧¬H9b), "calibration is a stable trait but unbiased" (¬H9a∧H9b), or "blindness concentrates at high stakes but the global bias is null" (H9c∧¬H9a). H9c is the load-bearing one: confirming it is direct evidence for the stakes/power discontinuity (`validity-threats.md` EV-4) — the instrument measuring its own extrapolation boundary. Exploratory **H9d** (calibration improves with use) is measurement-confounded in MVP-1 (practice vs genuine self-insight gain) and deferred to MVP-2.
